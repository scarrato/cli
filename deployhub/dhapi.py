...
def post_json_with_header(url, payload, headers):
    ...
    pprint(url)
    pprint(headers)
    lines = subprocess.run(["curl", "-X", "POST", "https://circleci.com/api/v2/project/github/ortelius/store-hipster-app-qa3-windows/pipeline", "-H", 'Accept: application/json', "-H", 'Circle-Token:2b88069948ec829dad359753d48de0b801f2d933', "-v" ], check=False, stdout=subprocess.PIPE).stdout.decode('utf-8').split("\n")
    for line in lines:
        print(line)
    return lines
...
def run_circleci_pipeline(pipeline):
    headers = {"Circle-Token": "2b88069948ec829dad359753d48de0b801f2d933", "Accept": "application/json"}
